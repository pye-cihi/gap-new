...
@Component({
  selector: 'app-graph-bsa',
  templateUrl: './graph-bsa.component.html',
  styleUrls: ['./graph-bsa.component.css']
})
export class GraphBsaComponent implements OnInit {
  public org_data = []
  public graph_data = []
  public cat = []
  ...
  public getSliderScore(data_list, id) {
    ...
    for(var i = 0; i < data_list.length; i++) {
      if(data_list[i].id == id && data_list[i].job_role == 2) {
