...
@Component({
  selector: 'app-graph-dev',
  templateUrl: './graph-dev.component.html',
  styleUrls: ['./graph-dev.component.css']
})
export class GraphDevComponent implements OnInit {
  public org_data = []
  public graph_data = []
  public cat = []
  ...
  public calculate_GAP(data_list, changeVal = 0, changeID = 0) {
    ...
    for (var i = 0; i < data_list.length; i++) {
      ...
      if(data_list[i].job_role == 1) { //if statement for dev only roles
  ...
  public getSliderScore(data_list, id) {
    ...
    for(var i = 0; i < data_list.length; i++) {
      if(data_list[i].id == id && data_list[i].job_role == 1) { //if statement for id and role check for devs only
